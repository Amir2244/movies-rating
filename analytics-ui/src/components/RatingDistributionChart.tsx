...
export const RatingDistributionChart: React.FC<{ data: RatingChartData[] }> = ({ data }) => {
    return (
        <Card className="bg-white/80 backdrop-blur-sm border-slate-300">
            <CardHeader>
                <CardTitle className="flex items-center gap-2 text-yellow-600"><BarChart className="h-5 "/>Detailed
                ...
                <ResponsiveContainer width="100%" height={300}>
                    <AreaChart data={data} margin={{top: 10, right: 30, left: 30, bottom: 0}}>
                        ...
                        <Tooltip contentStyle={{
                            ...
                            borderRadius: '8px'
                        }} formatter={(value) => [formatNumber(value as number), 'Ratings']}/>
                        <Area type="monotone" dataKey="count" stroke="#fbbf24" fill="url(#ratingGradient)"
                              strokeWidth={2}/>
                    </AreaChart>
                </ResponsiveContainer>
            </CardContent>
